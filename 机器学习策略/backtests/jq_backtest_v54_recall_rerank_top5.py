...
def initialize(context):
    # 核心观察模型 2：V5.4 召回 + 精排。
    # 当前观察配置：recall40 -> rerank prob Top5，纯月调仓。
    # 可测候选：
    #   recall40/top6/prob_topN
    #   recall40/top10/prob_x_recall_blend
    #   recall60/top8/prob_x_recall_blend
    #   baseline: use_baseline_recall_only=True, recall_top_k=30, final_top_n=10
    g.model_file = "model_candidate_v54_portfolio_oof_rerank_v410_q1.pkl"
    g.recall_top_k = 40
    g.final_top_n = 5
    ...
    if not isinstance(bundle, dict) or bundle.get("objective") != "v54_portfolio_oof_recall_rerank":
        raise ValueError("model bundle should be v54_portfolio_oof_recall_rerank")

    required_keys = [
        "recall_model",
        "recall_feature_cols",
        "recall_fill_values",
        "pool_relative_source_cols",
        "rerank_models",
    ]
    ...
    if g.use_baseline_recall_only:
        g.recall_top_k = 30
        g.final_top_n = 10
        g.selection_mode = "baseline_recall_score"

    g.recall_model = bundle["recall_model"]
    g.recall_feature_cols = list(bundle["recall_feature_cols"])
    g.recall_fill_values = dict(bundle.get("recall_fill_values", {}))
    g.pool_relative_source_cols = list(bundle.get("pool_relative_source_cols", []))
    g.prob_threshold = float(bundle.get("prob_threshold", 0.50))
    g.recall_blend_weight = float(bundle.get("recall_blend_weight", 0.35))

    g.rerank_model = None
    g.rerank_feature_cols = []
    g.rerank_fill_values = {}
    if not g.use_baseline_recall_only:
        rerank_key = "recall{}".format(g.recall_top_k)
        rerank_bundle = bundle.get("rerank_models", {}).get(rerank_key, None)
        if rerank_bundle is None:
            raise ValueError("rerank model missing for {}".format(rerank_key))
        g.rerank_model = rerank_bundle["rerank_model"]
        g.rerank_feature_cols = list(rerank_bundle["rerank_feature_cols"])
        g.rerank_fill_values = dict(rerank_bundle.get("rerank_fill_values", {}))

    # Only raw source features should be fetched from JoinQuant.
    # Rerank feature cols include generated pool-relative columns such as
    # poolrel_* and recall_rank_pct; those must be built after the recall pool
    # is formed, not requested from get_factor_values.
    g.feature_cols = unique_keep_order(g.recall_feature_cols + g.pool_relative_source_cols)
    ...
    g.universe_index = bundle.get("universe_index", "000906.XSHG")
    g.benchmark = bundle.get("benchmark", "000906.XSHG")
    g.stock_num = int(g.final_top_n)
    ...
    model_version = bundle.get("research_version", "v54_portfolio_oof_recall_rerank")
    price_mode = "v4_fill_paused" if g.requires_v4_feature_adapter else "legacy_skip_paused"
    log.info(
        "loaded V5.4 recall-rerank model %s: recall_features=%s rerank_features=%s "
        "recall_top_k=%s final_top_n=%s selection_mode=%s baseline=%s "
        "limit_up_sell=%s feature_adapter=%s price_mode=%s" % (
            model_version,
            len(g.recall_feature_cols),
            len(g.rerank_feature_cols),
            g.recall_top_k,
            g.final_top_n,
            g.selection_mode,
            g.use_baseline_recall_only,
...
def get_stock_list(context):
    ...
    df_factor["industry_bucket"] = pd.Series(industry_map_full).reindex(df_factor.index).fillna("UNKNOWN").astype(str)
    ...
    recall_pool = select_recall_pool(df_factor, g.recall_top_k)
    if recall_pool.empty:
        return []

    if g.use_baseline_recall_only:
        final_df = recall_pool.copy()
        final_df["final_score"] = final_df["recall_score_z"]
    else:
        final_df = add_pool_relative_features(recall_pool, g.pool_relative_source_cols)
        final_df = add_rerank_scores(final_df)
        final_df = calc_final_score(final_df)

    sorted_stocks = final_df.sort_values("final_score", ascending=False).index.tolist()
    industry_map = dict(zip(final_df.index, final_df["industry_bucket"]))
    ...
    log.info(
        "V5.4 recall_top_k=%s final_top_n=%s mode=%s baseline=%s" % (
            g.recall_top_k,
            g.final_top_n,
            g.selection_mode,
            g.use_baseline_recall_only,
...
def add_model_scores(df_factor):
    ...
    recall_X = out.reindex(columns=g.recall_feature_cols).replace([np.inf, -np.inf], np.nan)
    recall_X = recall_X.fillna(pd.Series(g.recall_fill_values)).fillna(0)
    out["recall_score"] = np.asarray(g.recall_model.predict(recall_X[g.recall_feature_cols])).reshape(-1)
    out["recall_score_z"] = zscore_series(out["recall_score"])
    return out


def select_recall_pool(df_factor, recall_top_k):
    if df_factor.empty:
        return df_factor.copy()
    candidate_count = min(int(recall_top_k), len(df_factor))
    out = df_factor.nlargest(candidate_count, "recall_score_z").copy()
    out = out.sort_values("recall_score_z", ascending=False)
    out["recall_rank"] = np.arange(1, len(out) + 1)
    out["recall_rank_pct"] = 1.0 - (out["recall_rank"] - 1.0) / max(1.0, len(out) - 1.0)
    out["recall_top_k"] = int(recall_top_k)
    return out


def add_pool_relative_features(pool_df, source_cols):
    out = pool_df.copy()
    out["recall_score_z_pool_rank"] = out["recall_score_z"].rank(pct=True)
    out["recall_score_z_minus_pool_median"] = out["recall_score_z"] - out["recall_score_z"].median()
    for col in source_cols:
        ...
        out[med_col] = out[col] - out[col].median()
        out[rank_col] = out[col].rank(pct=True)
        out[industry_rank_col] = out.groupby("industry_bucket")[col].transform(lambda s: s.rank(pct=True))
    return out.replace([np.inf, -np.inf], np.nan)


def add_rerank_scores(pool_df):
    out = pool_df.copy()
    if g.rerank_model is None or len(g.rerank_feature_cols) == 0:
        out["rerank_score"] = 0.0
        out["rerank_score_z"] = 0.0
        return out
    rerank_X = out.reindex(columns=g.rerank_feature_cols).replace([np.inf, -np.inf], np.nan)
    rerank_X = rerank_X.fillna(pd.Series(g.rerank_fill_values)).fillna(0)
    out["rerank_score"] = np.asarray(g.rerank_model.predict(rerank_X[g.rerank_feature_cols])).reshape(-1)
    out["rerank_score_z"] = zscore_series(out["rerank_score"])
    return out


def calc_final_score(pool_df):
    out = pool_df.copy()
    if g.selection_mode == "prob_topN":
        out["final_score"] = out["rerank_score_z"]
    elif g.selection_mode == "prob_threshold_then_topN":
        out["final_score"] = out["rerank_score_z"]
        low_prob = out["rerank_score"] < g.prob_threshold
        out.loc[low_prob, "final_score"] = -999.0 + out.loc[low_prob, "rerank_score_z"]
    elif g.selection_mode == "prob_x_recall_blend":
        out["final_score"] = out["rerank_score_z"] + g.recall_blend_weight * out["recall_score_z"]
    else:
        raise ValueError("unsupported V5.4 selection_mode: {}".format(g.selection_mode))
    return out
