...
# =========================
# V66 production-rule backtest config
# =========================
# 聚宽里通常把 pkl 上传到研究/回测文件的 test/ 目录；如果你上传到根目录，
# 只需要把 file 里的 "test/" 前缀删掉。
MODEL_CANDIDATES = [
    {
        "name": "v61_full_2019_2023_expanding60",
        "file": "test/model_candidate_v61_full_2019_2023_start20190101_cutoff20231231_fixed120.pkl",
        "note": "2019-2023 train, first OOS from 2024",
    },
    {
        "name": "v61_full_2019_2024_expanding72",
        "file": "test/model_candidate_v61_full_2019_2024_start20190101_cutoff20241231_fixed120.pkl",
        "note": "2019-2024 train, first OOS from 2025; V61/JQ-like best practical fixed anchor",
    },
    {
        "name": "v61_full_2019_2025_expanding84",
        "file": "test/model_candidate_v61_full_2019_2025_start20190101_cutoff20251231_fixed120.pkl",
        "note": "2019-2025 train, first OOS from 2026; latest expanding model",
    },
    {
        "name": "v46_original_2019_2025q1",
        "file": "test/model_candidate_v46_lgb_direct_hybrid_l2_ff10_2019_2025q1_legacy_unsealed.pkl",
        "note": "legacy anchor sanity check",
    },
    # 如果后续从 V65 导出 rolling60/rolling72 的 pkl，可直接加在这里：
    # {"name": "v65_rolling60m_latest", "file": "test/model_candidate_v65_rolling60m_latest.pkl", "note": "rolling60 latest"},
    # {"name": "v65_rolling72m_latest", "file": "test/model_candidate_v65_rolling72m_latest.pkl", "note": "rolling72 latest"},
]

# 每次在聚宽跑一个模型；复制回测后只改这个数字即可横向比较。
MODEL_INDEX = 1

PORTFOLIO_RULE = "top8_board_cap"
STOCK_NUM = 8
BOARD_CAPS = {"chinext": 3, "star": 2}
BOARD_CAPS_TEXT = "chinext:3;star:2"


def initialize(context):
    if MODEL_INDEX < 0 or MODEL_INDEX >= len(MODEL_CANDIDATES):
        raise ValueError("MODEL_INDEX out of range: {}".format(MODEL_INDEX))
    g.model_spec = MODEL_CANDIDATES[MODEL_INDEX]
    g.model_name = g.model_spec["name"]
    g.model_file = g.model_spec["file"]
    ...
    if "overlay_weight" in bundle:
        required_keys.append("overlay_weight")
    if not direct_mode:
    ...
    g.residual_feature_cols = list(bundle.get("residual_feature_cols", []))
    ...
    g.portfolio_rule = PORTFOLIO_RULE
    g.stock_num = int(STOCK_NUM)
    g.board_caps = dict(BOARD_CAPS)
    g.board_caps_text = BOARD_CAPS_TEXT
    ...
    log.info(
        "loaded V66 model sweep name=%s file=%s note=%s version=%s: base_features=%s residual_features=%s "
        "weight=%.2f mode=%s candidates=%s portfolio=%s stock_num=%s board_caps=%s "
        "limit_up_sell=%s feature_adapter=%s price_mode=%s" % (
            g.model_name,
            g.model_file,
            g.model_spec.get("note", ""),
            model_version,
...
def get_stock_list(context):
    ...
    target_list = build_targets(sorted_stocks, industry_map)

    log.info(
        "V66 sweep model=%s weight=%.2f mode=%s candidates=%s rule=%s stock_num=%s board_caps=%s" % (
            g.model_name,
            g.overlay_weight,
            g.overlay_mode,
            g.top_n_candidates,
    ...
    log_board_distribution(target_list)
...
def build_board_capped_targets(sorted_stocks, target_num, board_caps):
    ...
    # If the ranked list is too narrow after caps, fill by score order so the
    # portfolio stays fully invested. This mirrors the offline top8_board_cap rule.
    for stock in sorted_stocks:
        if stock not in selected:
            selected.append(stock)
            if len(selected) >= target_num:
...
def build_targets(sorted_stocks, industry_map):
    if g.portfolio_rule == "top8_board_cap":
        return build_board_capped_targets(
            sorted_stocks=sorted_stocks,
            target_num=g.stock_num,
            board_caps=g.board_caps,
        )
    if g.portfolio_rule == "industry_neutral":
    ...
    return sorted_stocks[:min(g.stock_num, len(sorted_stocks))]


def log_board_distribution(target_list):
    if len(target_list) == 0:
        return
    dist = {"main": 0, "chinext": 0, "star": 0}
    for stock in target_list:
        board = get_stock_board(stock)
        dist[board] = dist.get(board, 0) + 1
    log.info("board distribution: {}".format(dist))
